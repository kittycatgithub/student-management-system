...
const App = () => {
    ...
    const path1 = useLocation().pathname.includes('/ ')
    const path2 = useLocation().pathname.includes('/add-new')
    const path3 = useLocation().pathname.includes('/chat')

    const { showUserLogin, setShowUserLogin, user } = useAppContext()
    ...
    const sidebarLinks = [
        ...
        { name: "Add New", path: "/add-new", icon: overviewicon },
        ...
        <Toaster/>
            <div className="flex items-center justify-between px-4 md:px-8 border-b border-gray-300 py-3 bg-white transition-all duration-300">
                ...
                <div className="flex items-center gap-5 text-gray-500">
                    <p>Hi! Admin</p>
                    { user ? (
                        <button onClick={()=> setShowUserLogin(true)} className='border rounded-full text-sm px-4 py-1'>Logout</button>
                    ) : (
                        <button onClick={()=> setShowUserLogin(true)} className='border rounded-full text-sm px-4 py-1'>Login</button>
                    ) }
                    {/* <button onClick={()=> setShowUserLogin(true)} className='border rounded-full text-sm px-4 py-1'>Login</button> */}
                </div>
            </div>
            <div className="flex flex-row">
                           <div className="md:w-64 w-16 border-r h-[550px] text-base border-gray-300 flex flex-col transition-all duration-300">
            ...
            <div className="md:w-full w-full border-r h-[550px] text-base border-gray-300 flex flex-col transition-all duration-300">
               <Routes>
                  <Route path='/' element={<StudentList/>}/>
                  <Route path='/add-new' element={<NewStudent/>}/>
                  {/* <Route path='' element={<StudentList/>}/> */}
               </Routes>
            </div>
